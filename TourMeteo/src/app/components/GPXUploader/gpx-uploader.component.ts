...
export class GpxUploaderComponent {
  ...
  // Ride score & clothing recommendation
  rideScore = 0;
  rideScoreLabel = '';
  rideScoreColor = '';
  rideScoreEmoji = '';
  clothingItems: Array<{ emoji: string; label: string }> = [];
  rideWarnings: string[] = [];
  rideTips: string[] = [];
  avgTemp = 0;
  avgApparentTemp = 0;
  ...
  async computeSchedule() {
    ...
    // Compute ride score & clothing
    this.computeRideScore();

    this.cd.detectChanges();
  }

  // Compute ride score & clothing recommendations based on average weather across passages
  computeRideScore() {
    ...
    // --- Clothing recommendations based on apparent temperature ---
    this.clothingItems = [];

    // Cuissard (bib shorts)
    if (minApparent >= 18) {
      this.clothingItems.push({ emoji: '🩳', label: 'Cuissard court' });
    } else {
      this.clothingItems.push({ emoji: '🦵', label: 'Cuissard long' });
    }

    // Sous-maillot (base layer)
    if (minApparent >= 20) {
      // No base layer needed
      this.clothingItems.push({ emoji: '👕', label: 'Pas de sous-maillot' });
    } else if (minApparent >= 12) {
      this.clothingItems.push({ emoji: '🧵', label: 'Sous-maillot manches courtes' });
    } else {
      this.clothingItems.push({ emoji: '🧶', label: 'Sous-maillot manches longues' });
    }

    // Maillot (jersey)
    if (minApparent >= 15) {
      this.clothingItems.push({ emoji: '🚴', label: 'Maillot manches courtes' });
    } else {
      this.clothingItems.push({ emoji: '🚴‍♂️', label: 'Maillot manches longues' });
    }

    // Veste (jacket)
    if (minApparent < 8 || (this.hasRain && this.totalPrecip > 1)) {
      this.clothingItems.push({ emoji: '🧥', label: 'Veste avec manches' });
    } else if (minApparent < 14 || this.hasStrongWind) {
      this.clothingItems.push({ emoji: '🦺', label: 'Gilet coupe-vent (sans manches)' });
    }

    // Kawet (rain cape)
    if (this.hasRain && this.maxPrecipProb > 50) {
      this.clothingItems.push({ emoji: '🌧️', label: 'Kawet / cape de pluie' });
    }

    // Gants
    if (minApparent < 8) {
      this.clothingItems.push({ emoji: '🧤', label: 'Gants longs' });
    } else if (minApparent < 14) {
      this.clothingItems.push({ emoji: '🧤', label: 'Gants courts / mitaines' });
    }

    // Couvre-chaussures
    if (minApparent < 5 || (this.hasRain && this.totalPrecip > 2)) {
      this.clothingItems.push({ emoji: '🥾', label: 'Couvre-chaussures' });
    }

    // Bonnet / cache-oreilles sous casque
    if (minApparent < 5) {
      this.clothingItems.push({ emoji: '🎧', label: 'Sous-casque / cache-oreilles' });
    }

    // Lunettes
    if (codes.some(c => c === 0 || c === 1)) {
      this.clothingItems.push({ emoji: '🕶️', label: 'Lunettes de soleil' });
    }
